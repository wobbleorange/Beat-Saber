const trackNameL = "environmentL";
const trackNameR = "environmentR";
const mapInput = "D:\\Games\\SteamLibrary\\steamapps\\common\\Beat Saber\\Beat Saber_Data\\CustomWIPLevels\\test - enviro - Timba\\NormalStandard.dat"
const mapOutput = "D:\\Games\\SteamLibrary\\steamapps\\common\\Beat Saber\\Beat Saber_Data\\CustomWIPLevels\\test - enviro - Timba\\EasyStandard.dat"
...
map._notes.forEach(x => {
    if (x._customData && x._customData._track == trackNameL) {
        var y = copy(x);
        ...
        pillarScale[0] *= pillarToNoodleUnits * 4;
        pillarScale[1] *= pillarToNoodleUnits / 210;
        pillarScale[2] *= pillarToNoodleUnits * 30;
...
map._notes.forEach(x => {
    if (x._customData && x._customData._track == trackNameR) {
        var y = copy(x);
        ...
        pillarScale[0] *= pillarToNoodleUnits * 4;
        pillarScale[1] *= pillarToNoodleUnits / 140;
        pillarScale[2] *= pillarToNoodleUnits * 30;
...
map._notes = map._notes.filter(x => !x._customData || (x._customData && !x._customData._track == trackNameL));
map._notes = map._notes.filter(x => !x._customData || (x._customData && !x._customData._track == trackNameR));
