...
class MyProperties(PropertyGroup):
    ...
    opEnv: EnumProperty(
        items=[("walls", "Walls", "Export for Obstacles"),("notes", "Notes", "Export for Notes\n(also disables arrow, look and gravity)"),("bts", "BTS", "Export for BTS Environment"),("billie", "Billie", "Export for Billie Environment"),("gaga", "Gaga", "Export for Gaga Environment")],
        name="Output",
        description=":",
        default=None,
        )
...
class WM_OT_Blend2JSON(Operator):
    ...
    def execute(self, context):
        ...
        for sel in selection:
            ...
            if mytool.opEnv == "notes":
                cube = "{\"_time\":" + str(mytool.beat+NJSOffset) + ",\"_lineIndex\":0,\"_lineLayer\":0,\"_type\":1,\"_cutDirection\":8,\"_customData\":{\"_disableNoteGravity\":true,\"_disableNoteLook\":true,\"_noteJumpStartBeatOffset\":" + str(NJSOffset) +","
            else:
                cube = "{\"_time\":" + str(mytool.beat) + ",\"_lineIndex\":0,\"_type\":0,\"_duration\":" + str(mytool.dur) + ",\"_width\":0,\"_customData\":{"
            ...
            if scaleAnimExist == True or thick != 0.5 or mytool.opEnv == "notes":
                ...
                cube += scal + ','
                cube += defPos + ',' + locRot + '},"_scale":[2,2,2]}},\n'
            result += cube
